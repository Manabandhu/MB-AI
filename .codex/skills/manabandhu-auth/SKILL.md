...
3. The first-batch auth routes `/sign-in`, `/sign-up`, and `/forgot-password` use `frontend/src/modules/foundation/screens/StitchPrototypeScreens.tsx`; temporary demo login shows `demo@manabandhu.local` / `DemoPass123` and returns to `/home` until Supabase auth actions are wired.
