...
- **Auth Suite & Website Designs**: Added `AuthPageLayout` providing a responsive 2-column split-screen layout on desktop web (branded showcase with community pillars, testimonials, trust stats) and fluid mobile view. Added `signInWithOAuth` supporting Apple and Google social sign-in. Added dedicated `MagicLinkScreen` at route `/magic-link` with passwordless email sign-in and confirmation state. Upgraded `PhoneLoginScreen` (country selector chips, formatted phone input), `EmailLoginScreen` (visibility toggle, remember me, Apple & Google auth), `ForgotPasswordScreen` (email recovery card, countdown timer), `ResetPasswordScreen` (dynamic 4-bar password strength meter), `OtpVerificationScreen` (6-digit code input, resend countdown), and `ChooseLoginMethodScreen` (rich selector cards).
