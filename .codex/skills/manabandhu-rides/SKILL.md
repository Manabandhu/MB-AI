...
- Hardcoded data cleanup: removed `DRIVER_METADATA` and `RIDE_DETAILS_DATA` dictionaries containing fake personas ("Vikram Patel", "Sneha Murthy", etc.) in `RidesScreen` and `RideDetailScreen` in favor of dynamic presentation fields derived from `RideOffer`.
