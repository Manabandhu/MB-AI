...
- Real-Time Chat Engine & Broadcast Typing: Integrated `useRealtimeChat.ts` hook utilizing `supabase.channel` for instant <50ms Postgres Changes message delivery and ephemeral typing indicator broadcasts without polling. Enhanced `ConversationScreen` with optimistic message rendering, sticky bottom bar, and `expo-image-picker` photo attachments. Enhanced `ChatListScreen` with dedicated tabs for `Direct Messages` and `Temporary Carpools`. Upgraded `ConversationInfoScreen` with participant member lists, media gallery, and controls (`Mute Notifications`, `Leave Group`, `Report User`).
