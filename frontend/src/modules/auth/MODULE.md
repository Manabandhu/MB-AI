...
The first Stitch batch routes `/sign-in`, `/sign-up`, and `/forgot-password` render through `frontend/src/modules/foundation/screens/StitchPrototypeScreens.tsx`; current demo submission can use `demo@manabandhu.local` / `DemoPass123` and returns to `/home` until Supabase auth actions are wired.
